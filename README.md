...
코치 화면의 답변 아래 버튼입니다. 디렉터는 "좋다/아쉽다 + 한 줄"만 남깁니다.

| 버튼 | 플레이북 상태 | 승인 전에도 하는 일 |
|---|---|---|
| 👍 좋아요 | 답변작성 (승인 후보) | **다른 수강생의 비슷한 질문에 참고 근거로 검색**됨 (`chunks` origin `playbook`, verified `like`) · 자동 평가의 정답지 |
| 👎 아쉬워요 | 대기 (수정 후보) | 커밍쏜이 고쳐 승인하면 반영 |

👍 답변은 수강생 이름에 묶이지 않습니다. 검증된 판단 기준이 자산이지 이름이 자산이
아니기 때문입니다. 수강생 이름은 **그 수강생의 기억**(이전 상담 이어가기)에만 쓰입니다.
승인은 자주 오는 질문처럼 답을 고정할 가치가 있을 때만 하면 됩니다. 승인하면
프롬프트에 그대로 들어가고, 검색 근거로도 `approved` 로 표시됩니다.

커밍쏜은 플레이북 위 계기판에서 최근 30일 👍/👎, 검증 세트 규모, 검색 반영 현황을 봅니다.
🧪 자동 평가 → **🔁 검색 근거 재반영** 을 누르면 👍·승인 답변 전체를 다시 임베딩합니다
(승인 상태를 플레이북에서 바꾼 뒤엔 저장 시 자동 반영되므로 평소엔 필요 없습니다).
...
### 녹취 → 판단 카드 증류 (`api/distill.js`)

설정 › 디렉팅 사례 위의 패널. 컨설팅 녹취 140편(약 200만 자)에서 커밍쏜의 판단을
"상황 → 진단 → 처방 → 이유 + 원문 발화" 카드로 뽑는다. 초안(`cases.status='draft'`)은
커밍쏜이 **승인**해야 `aiApplied=true` 가 되고, 그때 `chunks`(origin `case`) 로 임베딩돼
질문과 의미가 가까운 카드 3장이 답변 근거에 붙는다(예전 단어 겹침 방식은 폴백).
3편씩 처리하며 `distill/{key}` 에 진행을 남기므로 끊겨도 이어서 된다. 추출 모델은
claude-sonnet-5(구조화 추출엔 충분, 비용 약 1/5).
