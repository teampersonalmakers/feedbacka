...
` : ''}`;
  ...
  // 컨설팅 논리 체크 — 커밍쏜이 컨설팅할 때 밟는 진단 순서·질문. 말투보다 이게 답변의 뼈대다.
  const logicBlock = (g.logicChecks || []).length
    ? '\n\n[커밍쏜 컨설팅 논리 체크 — 답하기 전에 이 순서로 참여자를 진단하고, 끊어진 고리를 찾아 거기에 답한다]\n' +
      g.logicChecks.map((l, i) => `${i + 1}. ${l}`).join('\n')
    : '';
...
[말투와 스타일]
${toneGuide}${feedbackOrder}${freeGuidelines}${doNotDo}${logicBlock}${GROUNDING}${quoteBlock}${categoryRules}${playbookStr}`;
